...
class xlsx2py:
    ...
    def constructMapDict(self):
        ...
        for col in range(0, self.xbook.getRowCount(self.mapIndex)):
            ...
            if colValues:
                for v in [e for e in colValues[1:] if e and e[0] and isinstance(e[0], str) and e[0].strip()]:
                    print(v)
                ...
                if len(reTuple) == 3:  # 定义被分拆为三部分:name, signs, func,signs可以是空
                    ...
                    if name not in enName:  # name不能重复
                        ...
                        self.xlsxClear(EXPORT_ERROR_REPEAT, \
                                       (self.xbook.getSheetNameByIndex(index).encode(FILE_CODE), EXPORT_DEFINE_ROW,
                                        c + 1))
                    ...
                    self.xlsxClear(EXPORT_ERROR_HEADER,
                ...
                for row in range(3, rows + 1):
                    ...
                    for col in range(1, cols + 1):
                        ...
                        if '$' in sign and len(val[0]) > 0:
                            ...
                            v = self.mapDict[
                                xlsxtool.GTOUC(xlsxtool.val2Str(val[0]))]  # mapDict:key是unicode.key都要转成unicode
                        else:
                            v = val[0]
